public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        String[] fragments = description.trim().split(";");
        String type="", name="";
        int length=0, allowance=0;
        double due=0, monthly=0, mile_unit=0, charger=0;
        Vehicle vehicle;
        Fuel fuel;
        Lease lease;

        for(int i=0; i<fragments.length; i++){
            String[] e = fragments[i].trim().split(":");
            if(e[0].equals("type"))
                type = e[1];
            else if(e[0].equals("name"))
                name = e[1];
            else if(e[0].equals("due"))
                due = Double.parseDouble(e[1]);
            else if(e[0].equals("length"))
                length = Integer.parseInt(e[1]);
            else if(e[0].equals("monthly"))
                monthly = Double.parseDouble(e[1]);
            else if(e[0].equals("mile/unit"))
                mile_unit = Double.parseDouble(e[1]);
            else if(e[0].equals("allowance"))
                allowance = Integer.parseInt(e[1]);
            else //e[0].equals("charger")
                charger = Double.parseDouble(e[1]);
        }

        // gas type
        if(fragments.length == 7){
            fuel = new Fuel(mile_unit);
            lease = new Lease(due, length, monthly, allowance);
            vehicle = new Vehicle(name, fuel, lease);
        }
        // e type
        else{
            fuel = new Fuel(mile_unit, charger);
            lease = new Lease(due, length, monthly, allowance);
            vehicle = new Vehicle(name, fuel, lease);
        }
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for(int i=0; i<vehicles.length; i++){
            ...
            if(vehicles[i].getFuel().getType()==1){ //GAS type
                vehicles[i].setCO2Emission(computeCO2(numberOfMonth, usage, mileageAllowance, 8.887));
                vehicles[i].setFuelCost(computeFuelCost(numberOfMonth, usage, mileageAllowance, gasPrice));
                vehicles[i].setTotalCost(vehicles[i].getFuelCost()+computeLeaseCost(dueAtSigning, numberOfMonth, monthlyCost));
            }
            else{ // e type
                vehicles[i].setCO2Emission(computeCO2(numberOfMonth, usage, mileageAllowance, 0.453));
                vehicles[i].setFuelCost(computeFuelCost(numberOfMonth, usage, mileageAllowance, electricityPrice));
                vehicles[i].setTotalCost(vehicles[i].getFuelCost()+computeLeaseCost(dueAtSigning, numberOfMonth, monthlyCost)+vehicles[i].getFuel().getCharger());
            }
        }
    }
